...
def get_deeppocket_residues(pdb):
	df = pd.read_csv('train_data/train_preds_results/DeepPocket_train_results.csv')
...
def get_grasp_residues(pdb):
	...
	df = pd.read_csv('train_data/train_preds_results/GRaSP_train_results.csv')
...
def get_p2rank_residues(pdb):
	...
	df = pd.read_csv('train_data/train_preds_results/p2Rank_train_results.csv')
	results = df.loc[df['Protein'] == pdb, 'Binding_site'].values
	all_results = []
...
def get_pointsite_residues(pdb):
	...
	df = pd.read_csv('train_data/train_preds_results/PointSite_train_results.csv')
	results = df.loc[df['Protein'] == pdb, 'Binding_Site'].values
	all_results = []
...
def get_puresnet_residues(pdb):
	...
	df = pd.read_csv('train_data/train_preds_results/PUResNet_train_results.csv')
...
def get_labels():
	file = open('train_data/train_binding_labels_formatted.txt')
...
if __name__ == '__main__':
	...
	dataset = 'train_data/train_coach_pdbs'
	...
	final_result_file = open('train_final_data.csv', 'w')
